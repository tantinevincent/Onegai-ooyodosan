...
class DockingRunner(Common):
    def __init__(self, docker_num, fleets):
    ...
    def run(self):
        ...
        wait("base.png",20)
        empty_docker_num = self.__get_empty_docker_num()
        if empty_docker_num == 0:
            self.back_home_port()  
            return True
        
        self.__click_docker()
        ships = self.__list_repairing_candidate(empty_docker_num <= 1)
        ...
        for ship in ships:
            ...
            if not success:
                continue
            if self.__get_empty_docker_num() == 0:
                break
            self.__click_docker()
    ...
    @logged
    def __get_empty_docker_num(self):
    ...
    @logged
    def __list_repairing_candidate(self, is_fleet_only):
        ships = self.__get_in_fleets_ships()               
        if not is_fleet_only:
...
if __name__ == "__main__":
    runner = DockingRunner(2, [Fleet(1)])
    runner.run()